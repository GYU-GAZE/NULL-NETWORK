...
const ONBOARDING_STARTER_LEVEL: int = 3
...
func create_partner_state(
	...
	partner.nickname = nickname.strip_edges() if not nickname.strip_edges().is_empty() else apk.display_name
...
func create_tamed_partner_state(
	...
	var partner: PartnerStateData = create_partner_state(apk_id, "", individual_roll, individual_roll)
	if partner == null:
		return null
...
func evolve_partner(target_apk_id: String) -> PackedStringArray:
	...
	source.current_hp = clampi(roundi(float(new_stats.get("max_hp", 1)) * hp_ratio), 1 if hp_ratio > 0.0 else 0, int(new_stats.get("max_hp", 1)))
...
func select_starter(
	...
	if not errors.is_empty():
		return errors
	var partner: PartnerStateData = create_partner_state(apk_id, nickname, personality_roll, address_term_roll)
	if partner == null:
		errors.append("Starter APK '%s' is invalid or not registered." % apk_id)
		return errors
	var apk: APKData = ContentRegistry.get_apk(apk_id)
	_configure_onboarding_starter_state(apk, partner)
...
func _configure_onboarding_starter_state(apk: APKData, partner: PartnerStateData) -> void:
	if apk == null or partner == null:
		return
	partner.level = ONBOARDING_STARTER_LEVEL
	partner.current_exp = get_total_exp_for_level(ONBOARDING_STARTER_LEVEL)
	partner.allocation_points = maxi(0, ONBOARDING_STARTER_LEVEL - 1)
	# A level-3 starter must already know authored rewards unlocked at levels 2/3.
	for reward: APKLevelRewardData in apk.learnable_modules:
		if reward == null or reward.required_level > ONBOARDING_STARTER_LEVEL:
			continue
		for module_id: String in reward.active_module_ids:
			if not partner.known_active_module_ids.has(module_id):
				partner.known_active_module_ids.append(module_id)
	var stats := APKStatCalculator.calculate_stats(apk, partner)
	partner.current_hp = int(stats.get("max_hp", 1))
...
func validate_partner_state(partner: PartnerStateData) -> PackedStringArray:
	...
	if not partner.secondary_passive_module_id.is_empty() and ContentRegistry.get_module(partner.secondary_passive_module_id) == null:
		errors.append("Partner references an unknown secondary passive Module.")
	if not partner.growth_lineage.is_empty() and ContentRegistry.get_apk(partner.growth_lineage) == null:
	...
	if partner.current_exp < minimum_exp or (partner.level < MAX_LEVEL and partner.current_exp >= next_exp):
...
func commit_combat_snapshot(actor: Dictionary, experience_gain: int = 0) -> PackedStringArray:
	...
	CampaignState.partner.current_hp = clampi(roundi(float(actor.get("hp", 0.0))), 0, int(stats_before.get("max_hp", 1)))
	CampaignState.partner.current_stability = clampi(roundi(float(actor.get("stability", 0.0))), 0, PartnerStateData.MAX_STABILITY)
	...
	for module_id: String in module_ids:
		if not CampaignState.partner.known_active_module_ids.has(module_id):
			modules_known = false
			break
	if module_ids.size() == ACTIVE_SLOT_COUNT and not module_ids.has("") and modules_known:
...
func grant_experience(amount: int) -> PackedStringArray:
	...
	if CampaignState.partner.level != old_level:
		...
		CampaignState.partner.current_hp = clampi(CampaignState.partner.current_hp + hp_difference, 1 if CampaignState.partner.current_hp > 0 else 0, int(new_stats.get("max_hp", 1)))
...
func allocate_stat(stat_id: String, amount: int = 1) -> PackedStringArray:
	...
	if not errors.is_empty():
		return errors
	var old_stats: Dictionary = get_current_stats()
	CampaignState.partner.allocated_stats[clean_stat] = CampaignState.partner.get_allocated_stat(clean_stat) + amount
