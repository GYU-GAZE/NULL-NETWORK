...
class_name OccupationConditionData
...
@export var requirement: Requirement = Requirement.MATCHES
...
func _evaluate_rule(
	...
	var current_id: String = CampaignState.operator.occupation_id.strip_edges()
	var expected_id: String = occupation_id.strip_edges()
...
func _validate_rule(_active_path: Dictionary) -> PackedStringArray:
	...
	if requirement in [Requirement.MATCHES, Requirement.DOES_NOT_MATCH] \
		and occupation_id.strip_edges().is_empty():
		errors.append("occupation_id is required for the selected requirement.")
