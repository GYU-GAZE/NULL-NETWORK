...
| DVS-022 | **Superseded by DVS-028.** Ordinary defeat remained recoverable only while Phase 10 lacked the irreversible lifecycle. | Do not restore the old generic one-HP policy for primary partners. Only the temporary TURD safeguard remains until Operator Loss lands. | Historical Phase 10 boundary |
...
| DVS-032 | TURD reaching 0 HP causes Operator Loss. | TURD temporarily retains the one-HP compatibility safeguard only until the Operator Loss transition is implemented; primary partners do not. | GDD: Operator Loss; Phase 14 plan |
| DVS-033 | Operator Loss does not reset campaign time or world consequences. | New Operator creation must preserve `TimeManager`, infestation, completed events and other world-owned state while resetting identity-owned state. | GDD: Operator Loss |
