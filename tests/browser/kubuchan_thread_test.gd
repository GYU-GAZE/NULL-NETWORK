...
func _run_test() -> void:
	...
	var human_mentions_wewere := false
	var human_mentions_missing_people := false
	var human_mentions_scrapped_origin := false
	var human_calls_it_admin_bot := false

	for post_value in THREAD_DATA.posts:
		var post := post_value as KubuchanPostData
		if post == null:
			continue
		var lowered := post.body_text.to_lower()
		...
		human_mentions_scrapped_origin = human_mentions_scrapped_origin or lowered.contains("cancelled") or lowered.contains("scrap")
		human_calls_it_admin_bot = human_calls_it_admin_bot or (lowered.contains("admin") and lowered.contains("bot"))
	...
	_check(site.find_child("ThreadBar", true, false) == null, "Kubuchan must not regress to a forum-style separate thread title bar.")
	...
	_check(site.find_child("ThreadScroll", true, false) == null, "Kubuchan must not put only the post list inside a nested thread scroller.")
	if page_scroll != null:
		var vertical_bar: VScrollBar = page_scroll.get_v_scroll_bar()
		_check(vertical_bar != null and vertical_bar.max_value > page_scroll.size.y, "Kubuchan content must exceed the viewport and expose a vertical scroll range.")
	...
	if cards.size() >= 2:
		...
		var op_style := op_panel.get_theme_stylebox(&"panel") as StyleBoxFlat if op_panel != null else null
		var reply_style := reply_panel.get_theme_stylebox(&"panel") as StyleBoxFlat if reply_panel != null else null
		_check(op_style != null and op_style.bg_color.a == 0.0, "Classic imageboard OP must sit directly on the page instead of inside a forum card.")
		_check(reply_style != null and reply_style.bg_color.a > 0.0, "Imageboard replies must use compact tinted reply boxes.")
		_check(reply_indent != null and reply_indent.custom_minimum_size.x > 0.0, "Imageboard replies must be indented relative to the OP.")
		_check(op_panel != null and reply_panel != null and reply_panel.custom_minimum_size.x < op_panel.custom_minimum_size.x, "Replies must remain content-sized boxes instead of full-width forum cards.")

		var timestamp_label := cards[0].find_child("TimestampLabel", true, false) as Label
		var expected_date_prefix := "%02d/%02d/%02d(" % [
			TimeManager.current_month_index + 1,
			TimeManager.current_calendar_day,
			posmod(TimeManager.current_year, 100)
		]
		_check(timestamp_label != null and timestamp_label.text.begins_with(expected_date_prefix), "Kubuchan timestamps must use the current in-game calendar date.")

		var body_label := cards[1].find_child("BodyLabel", true, false) as SiteRichTextLabel
		_check(body_label != null, "Kubuchan post bodies must use SiteRichTextLabel for reusable local references.")
		if body_label != null:
			_check(body_label.text.contains("ref:post:"), "Kubuchan quote markup must resolve to local post references.")
			body_label.local_reference_requested.connect(_on_reference_requested)
			body_label._run_meta_action("ref:post:%d" % (THREAD_DATA.posts[0] as KubuchanPostData).post_number)
			_check(_reference_value.begins_with("post:"), "SiteRichTextLabel did not emit the local reference action.")

	var visible_buttons: Array[BaseButton] = []
	_collect_visible_buttons(site, visible_buttons)
	_check(visible_buttons.size() == 1, "Kubuchan must expose exactly one external navigation button; quotes are internal page references.")
	if visible_buttons.size() == 1:
		_check(visible_buttons[0] is LinkButton, "Kubuchan's external navigation must look like a normal imageboard hyperlink.")
		visible_buttons[0].pressed.emit()
		_check(_navigated_url == "null.net", "Kubuchan's external link did not navigate to null.net.")
...
func _collect_visible_buttons(root: Node, output: Array[BaseButton]) -> void:
	if root is BaseButton:
		var button := root as BaseButton
		if button.visible and not button.disabled:
			output.append(button)
	for child: Node in root.get_children():
		_collect_visible_buttons(child, output)

func _on_navigation_requested(url: String) -> void:
	_navigated_url = url

func _on_reference_requested(reference: String) -> void:
