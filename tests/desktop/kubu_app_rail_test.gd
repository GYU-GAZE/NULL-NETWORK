...
func _run_test() -> void:
	...
	if rail != null:
		...
		_check(
			is_equal_approx(KubuOSMetrics.reserved_left_width, rail.rail_width),
			"App rail did not reserve its logical width in KubuOSMetrics."
		)
		_check(
		...
		_check(
			is_equal_approx(rail.sidebar_margin.offset_right, rail.rail_width),
			"App rail scene width disagrees with its work-area reservation."
		)

		var work_rect: Rect2 = KubuOSMetrics.get_work_area_rect(size)
		_check(
			is_equal_approx(work_rect.position.x, rail.rail_width),
			"Workspace did not move to the right of the app rail."
		)
		_check(
