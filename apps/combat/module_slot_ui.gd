...
func setup(
	...
	if style != null:
		ui_style = style
	_apply_style()
	label.text = module.module_name if module != null else "EMPTY"
...
func _apply_style() -> void:
	...
	row.add_theme_constant_override("separation", ui_style.module_separation)
	add_theme_stylebox_override(
		"panel",
		ui_style.copy_style(ui_style.module_slot_style)
	)
	ui_style.apply_font(label, ui_style.module_font, ui_style.module_font_size)
...
func _get_drag_data(_at_position: Vector2) -> Variant:
	...
	if ui_style != null:
		ui_style.apply_font(preview, ui_style.module_font, ui_style.module_font_size)
...
func _can_drop_data(_at_position: Vector2, data: Variant) -> bool:
	return data is Dictionary and data.get("module") is ModuleData

func _drop_data(_at_position: Vector2, data: Variant) -> void:
	var source := data.get("source_slot") as ModuleSlotUI
	var dragged_module := data.get("module") as ModuleData
	if source == null or dragged_module == null:
		return
	if is_equipped_slot:
		if source.is_equipped_slot:
			CombatManager.set_player_module(source.slot_index, current_module)
		CombatManager.set_player_module(slot_index, dragged_module)
	elif source.is_equipped_slot:
		CombatManager.set_player_module(source.slot_index, null)
	modules_changed.emit()

func _on_hover_in() -> void:
	if current_module == null:
		return
	tooltip_requested.emit(ModuleDescriptionService.build_tooltip(current_module))
