...
func _setup_profile() -> void:
	if post_data.avatar != null:
		avatar_rect.texture = post_data.avatar
		avatar_rect.show()
	else:
		avatar_rect.hide()
...
func _build_username_text() -> String:
	...
	if post_data.is_moderator:
		parts.append("MOD")

	if post_data.is_system_post:
		parts.append("SYSTEM")
